...
import { deleteImageFromSupabase, uploadImageToSupabase, } from "../../helper/supabaseUploadFile.js";
import { createData, deleteData, getAll, getSingleData, updateData, } from "../../services/generalServices.js";
export const getCompData = async (req, res, next) => {
    try {
        ...
        if (result) {
            ...
            return next(ApiError.notFound("Data not found, or you do not have permission to access this page."));
        }
    }
    catch (error) {
...
export const getCompById = async (req, res, next) => {
    try {
        const userId = req.user?.id;
...
export const createComp = async (req, res, next) => {
    try {
...
export const updateComp = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        const id = req.params.id;
        const { template_id, header, image_url, logo, slogan, navbar, contact_info } = req.body;
...
export const deleteCompData = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        const id = req.params.id;
        const foundedData = await getSingleData(id, "simple_components", userId);
...
export const copyComponent = async (templateId, newTemplateId, userId) => {
    try {
