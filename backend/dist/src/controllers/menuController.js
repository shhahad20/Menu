...
export const createMenu = async (req, res, next) => {
    const { name, user_id, sections } = req.body;
    ...
    try {
        ...
        const itemsData = sections.flatMap((section, index) => section.items.map((item) => ({
        ...
        for (const section of sections) {
            ...
            if (section_id) {
                ...
                const { error: sectionError } = await supabase
                    ...
                    .eq("template_id", id);
                if (sectionError) {
                    return next(ApiError.internal(`Error updating section '${header}': ${sectionError.message}`));
                }
            }
            else {
                // Add new section
                const { data: newSection, error: newSectionError } = await supabase
                    ...
                    .single();
                if (newSectionError) {
                    return next(ApiError.internal(`Error adding new section '${header}': ${newSectionError.message}`));
                }
            ...
            for (const item of items || []) {
                ...
                if (item_id) {
                    ...
                    if (itemError) {
                        return next(ApiError.internal(`Error updating item '${title}' in section '${header}': ${itemError.message}`));
                    }
                }
                else {
                    // Add new item
...
export const copyMenuTemplate = async (req, res, next) => {
    ...
    try {
        ...
        const { error: updateError } = await supabase
            ...
            .eq("id", newTemplateId);
        if (updateError) {
            return next(ApiError.internal(`Error updating template with new component_id: ${updateError.message}`));
        }
        res.status(201).json({
