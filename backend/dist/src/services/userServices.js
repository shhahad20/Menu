...
import { registeringEmail } from '../helper/emails.js';
...
export const sendRegistrationEmail = async (email, firstName, lastName, token) => {
    try {
        // Prepare the email content
        const emailContent = registeringEmail(email, firstName, lastName, token);
        // Send the email
        await emailSender(emailContent);
        return { success: true, message: 'Email sent successfully', token };
    }
    catch (error) {
...
export const getUsers = async (page = 1, limit = 5, search = '') => {
    ...
    const users = await User.find(filter, ignoreOptions)
        .populate('order')
        .skip(skip)
        .limit(limit)
...
export const findUserAndDelete = async (id) => {
    try {
...
export const updateBanStatusById = async (id, isBanned) => {
...
export const updateRoleStatusById = async (id, isAdmin) => {
